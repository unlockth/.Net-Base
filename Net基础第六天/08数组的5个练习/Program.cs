...
namespace _08数组的5个练习
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //int sum = 0;
            ////循环的让数组中的每个元素跟我的最大值、最小值、进行比较
            //for (int i = 0; i < nums.Length; i++)
            //{
            //    //关于在循环中nums[i]的理解方式
            //    //1、代表数组中当前循环的元素
            //    //2、代表数组中的每个元素
            //    //如果数组中当前循环到的这个元素比我的max还要大，则把当前这个元素赋值给我的max，如果当前这个元素比我的min还要小，则把这个元素给我的min
            //    if (nums[i] > max)
            //    {
            //        max = nums[i];
            //    }
            ...
            ////老杨|老苏|老邹|老虎|老牛|老马
            ////解题思路：通过一个循环遍历数组中的每一个元素，然后将每一个元素累加到一个字符串中，通过|分割
            //string str = null;//""
            ...
            ////解题思路：遍历这个数组，让当前元素和0比较，比0大则加1，比0小则减1，如果等于0则不变
            //for (int i = 0; i < nums.Length; i++)
            //{
            //    if (nums[i] > 0)
            //    {
            ...
            //练习5：将一个字符串数组的元素顺序进行反转，{ "我","是","好人"} { "好人","是","我"}
            string[] names = { "我", "是", "好人" };
